/*************************************************************************/
/* Program: 02c_Simulation_Summary_BC.sas 
/* Creation Date: 18DEC2020
/* Purpose: Create table with simulation summary results for correctly specified models.
...
*N=100;
%readdat(25,100,02,BC);
%readdat(50,100,02,BC);
%readdat(75,100,02,BC);

*N=200;
%readdat(50,200,02,BC);
%readdat(100,200,02,BC);
%readdat(150,200,02,BC);

*N=500;
%readdat(125,500,02,BC);
%readdat(250,500,02,BC);
%readdat(375,500,02,BC);
...
proc report data=sim_table_BC headskip missing split="$"  style(column)=[font_size=8pt] nowindows ;
	  columns Pop_size sample_size Method Est_Tot /*ebias*/ pct_bias avg_se emp_se SE_ratio Cov2 CI_HW PI_Cov mean_RSE; 
	  define Pop_size / group order=data;
	  define sample_size / group order=data;
	  format Est_Tot CI_HW Cov2 10.0 ebias avg_se emp_se 10.1  PI_Cov SE_ratio mean_RSE 10.2 PCT_bias percentn10.2;
run ;
