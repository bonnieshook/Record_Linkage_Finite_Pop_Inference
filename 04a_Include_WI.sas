...
proc reg data=mockdata_iter&iter. outest=varcov&iter. covout;
 model P_I=urbannumcol SQRT_ICR;
...
data X_75&iter.;
 retain  intercept urbannumcol SQRT_ICR;
 set predvalues_&iter.;
 if response=0;
 intercept=1;
 keep intercept urbannumcol SQRT_ICR;
run;
...
proc iml;
varNames = {"Intercept" "urbannumcol" "SQRT_ICR"};
use X_75&iter.; 
...
PROC WTADJUST DATA=mockdata_resp_iter&iter. DESIGN=WR ADJUST=POST noprint MAXITER=100 P_EPSILON=0.001;
 ...
 NEST _one_;
 CLASS urbannumcol;
 MODEL response= urbannumcol SQRT_ICR;
 POSTWGT &ctrl_tot.;
