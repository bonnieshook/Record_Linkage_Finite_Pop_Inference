...
*Randomize response status onto frame, and remove data for non-selected counties;
...
proc reg data=mockdata_iter&iter. outest=varcov&iter. covout;
 model P_I=urbannumcol sqrt_pov;
...
%let df=%SYSEVALF(&n-3); *df for the t-value are n-1-p = n-3;
data _NULL_;
...
data X_75&iter.;
 retain  intercept urbannumcol sqrt_pov;
 set predvalues_&iter.;
 if response=0;
 intercept=1;
 keep intercept urbannumcol sqrt_pov;
run;
...
data Var&iter.;
 set varcovb&iter.;
 keep Intercept--sqrt_pov;
run;

proc iml;
varNames = {"Intercept" "urbannumcol" "sqrt_pov"};
use X_75&iter.; 
...
PROC WTADJUST DATA=mockdata_resp_iter&iter. DESIGN=WR ADJUST=POST noprint MAXITER=100 P_EPSILON=0.001;
 ...
 NEST _one_;
 CLASS urbannumcol;
 MODEL response= urbannumcol sqrt_pov;
 POSTWGT &ctrl_tot.;
